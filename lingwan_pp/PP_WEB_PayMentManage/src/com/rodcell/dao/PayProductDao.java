...
//					this.getMemcachedManager().cacheObject(tablename+payProduct.getProduct_name()+payProduct.getSid()+payProduct.getProduct_currency(), JSONUtil.objectToString(payProduct), this.getMemcachedManager().CACHE_EXP_FOREVER);
	...
	public PayProduct findPayProductByName(String name,long sid,String currency){
		PayProduct PayProduct = null;
		try {
...
//		Map m = JSONUtil.objToMap(payProduct);
		SqlXML sqlxml = getSql("insertProduct", par);
		
		long i = this.getDbManagement().insertAndGetKey(sqlxml.getSearchsql(), sqlxml.getPar());
//		if(i>0){
//			par.put("product_id", i+"");
//			this.getMemcachedManager().cacheObject(tablename+MapsUtil.getString(par, "product_id"), JSONUtil.objectToString(par), this.getMemcachedManager().CACHE_EXP_FOREVER);
//			this.getMemcachedManager().cacheObject(tablename+MapsUtil.getString(par, "product_name")+MapsUtil.getString(par, "sid"), JSONUtil.objectToString(par), this.getMemcachedManager().CACHE_EXP_FOREVER);
//			
//		}
		return i;
...
//		Map m = JSONUtil.objToMap(payProduct);
		SqlXML sqlxml = getSql("updateProduct", par);
