 ...
 */
@Controller
public class McscSmsController extends BaseAction{
	
	@Autowired
	PayMessageService originService;
	
	private static long paychannelid=43;
	
	//http://127.0.0.1/pay/order?gName=s1&uID=1&productId=1&cparam=test&payid=42&source=0
	@RequestMapping("/mcscSmsMO.do")  //订单创建接口
    public String thailandSmsMO(HttpServletRequest request, HttpServletResponse response, HttpSession session) {  
		...
		try {
			...
			log.info("mcscSmsMO.class uuid="+s+" parameter=="+JSONUtil.objectToString(map));
			...
			log.info("mcscSmsMO.class uuid="+s+" return=="+text+" runtime ="+(System.currentTimeMillis()-s));
			try{
	...
	@RequestMapping("/mcscSmsDRL.do")  //订单创建接口
    public String thailandSmsDRL(HttpServletRequest request, HttpServletResponse response, HttpSession session) {  
		Map map=null;
		try {
			long s=System.currentTimeMillis();
			map = getPar(paychannelid,request);
			log.info("mcscSmsDRL.class uuid="+s+" parameter=="+JSONUtil.objectToString(map));
			...
			log.info("mcscSmsDRL.class uuid="+s+" return=="+text+" runtime ="+(System.currentTimeMillis()-s));
			try{
				if(!status.contains(ErrorCode.RESPONSE200+"")){
					new SException(null, status, o);
				}
