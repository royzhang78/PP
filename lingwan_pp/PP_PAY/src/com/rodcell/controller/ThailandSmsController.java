 ...
 */
@Controller
public class ThailandSmsController extends BaseAction{
	
	@Autowired
	PayMessageService originService;
	
	private static long paychannelid=40;
	
	//http://127.0.0.1/pay/order?gName=s1&uID=1&productId=1&cparam=test&payid=42&source=0
	@RequestMapping("/thailandSmsMO.do")  //订单创建接口
    public String thailandSmsMO(HttpServletRequest request, HttpServletResponse response, HttpSession session) {  
		...
		try {
			...
			log.info("thailandSmsMO.class uuid="+s+" parameter=="+JSONUtil.objectToString(map));
			...
			log.info("thailandSmsMO.class uuid="+s+" return=="+text+" runtime ="+(System.currentTimeMillis()-s));
	...
	@RequestMapping("/thailandSmsDRL.do")  //订单创建接口
    public String thailandSmsDRL(HttpServletRequest request, HttpServletResponse response, HttpSession session) {  
		Map map =null;
		try {
			long s=System.currentTimeMillis();
			map = getPar(paychannelid,request);
			log.info("thailandSmsDRL.class uuid="+s+" parameter=="+JSONUtil.objectToString(map));
			...
			log.info("thailandSmsDRL.class uuid="+s+" return=="+text+" runtime ="+(System.currentTimeMillis()-s));
