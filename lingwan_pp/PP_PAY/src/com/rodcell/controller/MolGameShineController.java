 ...
 */
@Controller
public class MolGameShineController extends BaseAction{
	
	@Autowired
	PayMessageService originService;
	
	private static long paychannelid=46;
	
	//http://127.0.0.1/pay/order?gName=s1&uID=1&productId=1&cparam=test&payid=42&source=0
	@RequestMapping("/molGameShine.do")  //订单创建接口
    public String molGameShine(HttpServletRequest request, HttpServletResponse response, HttpSession session) {  
		Map map=null;
		try {
			long s=System.currentTimeMillis();
			map=getPar(paychannelid,request);
			log.info("molGameShine.class uuid="+s+" parameter=="+JSONUtil.objectToString(map));
			...
			log.info("molGameShine.class uuid="+s+" return=="+text+" runtime ="+(System.currentTimeMillis()-s));
			try{
	...
	@RequestMapping("/molgameshineSuccess.do")  //订单创建接口
    public String molgameshineSuccess(HttpServletRequest request, HttpServletResponse response, HttpSession session) {  
		Map map =null;
		try {
			long s=System.currentTimeMillis();
			map = getPar(paychannelid,request);
			log.info("molgameshineSuccess.class uuid="+s+" parameter=="+JSONUtil.objectToString(map));
			map.put("channelId", paychannelid+"");
			...
			log.info("molgameshineSuccess.class uuid="+s+" return=="+text+" runtime ="+(System.currentTimeMillis()-s));
			try{
				if(!status.contains(ErrorCode.RESPONSE200+"")){
					new SException(null, status, o);
				}
